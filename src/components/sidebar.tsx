...
const SideBar: React.FC<Props> = ({ children }) => {
	...
	const Filtered = SideBarData.filter((val) =>
		val.name.toLocaleLowerCase().includes(sidebarInput),
	);
						...
						<ul className="space-y-2">
							<li>
								...
								>
									...
									<span className="font-bold flex items-center gap-3 ml-3 ">
										OPTIMIZED REACT
									</span>
								</Link>
							</li>
							<li>
								<Input
									...
									value={sidebarInput}
									onChange={(e: any) =>
										setSidebarInput(e.target.value.toLowerCase())
									}
									placholder="Search"
									isSerach
								/>
